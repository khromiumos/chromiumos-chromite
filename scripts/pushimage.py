...
def PushImage(src_path, board, versionrev=None, profile=None, priority=50,
  ...
  try:
    ...
    cros_build_lib.Warning('Missing base instruction file: %s', e)
  ...
  for channel in channels:
    ...
    for image_type, dst_name, suffix in files_to_sign:
      try:
        ...
        cros_build_lib.Info('Nothing to sign: %s', e)
        continue
      ...
      if not keysets:
        ...
        if not keysets:
          cros_build_lib.Warning('Skipping %s image signing due to no keysets',
                                 image_type)
        ...
        with tempfile.NamedTemporaryFile(
          ...
          try:
            ...
            cros_build_lib.Error('Unknown error while marking for signing %s',
                                 gs_insns_path, exc_info=True)
            continue
          cros_build_lib.Info('Signing %s image with keyset %s at %s',
                              image_type, keyset, gs_insns_path)
